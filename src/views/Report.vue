  ...
  <div>
    <div class="navbar" style="margin-bottom:10px;">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>核酸报告管理</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
...
<!--      </el-table-column>-->
<!--&lt;!&ndash;      <el-table-column prop="place" label="就诊医院" width="200">&ndash;&gt;-->
<!--      </el-table-column>-->


      <el-table-column prop="register" label="登记时间" width="200">
      </el-table-column>
      <el-table-column prop="depart" label="风险类型" width="200">
      </el-table-column>
            <el-table-column prop="place" label="核酸检测结果" width="auto">
            </el-table-column>
...
<!--            </el-date-picker>-->
<!--          </el-form-item>-->
          <el-form-item label="核酸检测结果" :label-width="formLabelWidth">
...
export default {
  methods: {
    search() {
      if (!this.stext) {
        axios.get("http://localhost:8080/report/findAll/1/6").then((resp) => {
        ...
        axios
          .get(
            "http://localhost:8080/report/search/" +
              this.searchKey +
              "/" +
              this.stext
          )
    ...
    submitForm() {
      axios
        .post("http://localhost:8080/report/save", this.addEmpIden)
        .then((resp) => {
          if (resp.data == "success") {
            this.$alert("核酸报告信息添加成功！", "消息", {
    ...
    deleteRecord(row) {
      this.$confirm("是否确定要删除" + row.name + "的人员信息记录?", "删除数据", {
        ...
        axios
          .delete("http://localhost:8080/report/deleteById/" + row.id)
          .then((resp) => {
            this.$alert(row.name + "核酸报告信息删除成功！", "消息", {
              confirmButtonText: "确定",
              callback: (action) => {
                window.location.reload();
              },
    ...
    update() {
      axios
        .put("http://localhost:8080/report/update", this.EmpIden)
        .then((resp) => {
          console.log(resp);
          if (resp.data == "success") {
            this.$alert(this.EmpIden.name + "核酸报告信息修改成功！", "消息", {
              confirmButtonText: "确定",
              callback: (action) => {
                window.location.reload();
              },
    ...
    edit(row) {
      axios
        .get("http://localhost:8080/report/findById/" + row.id)
        .then((resp) => {
    ...
    handleCurrentChange(currentPage) {
      axios
        .get("http://localhost:8080/report/findAll/" + currentPage + "/6")
        .then((resp) => {
          this.tableData = resp.data.records;
          this.total = resp.data.total;
        });
